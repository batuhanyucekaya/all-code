    ...
    public class SepetController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetMusteriSepeti(int musteriId)
        {
            try
            {
                ...
                _logger.LogInformation($"{musteriId} ID'li müşterinin sepeti listelendi. {sepetUrunleri.Count} kayıt bulundu.");
        ...
        public async Task<IActionResult> SepeteEkle([FromBody] SepetEkleRequest request)
        {
            ...
            try
            {
                ...
                var sepetKaydi = new Sepet
                {
                    MusteriId = request.MusteriId,
                    SiparisTarihi = DateTime.UtcNow
                };

                await _context.Sepet.AddAsync(sepetKaydi);
                await _context.SaveChangesAsync();

                _logger.LogInformation($"{request.MusteriId} ID'li müşterinin sepetine yeni kayıt eklendi.");
                return Ok(new { message = "Sepete eklendi", id = sepetKaydi.Id });
            }
            catch (Exception ex)
            {
        ...
        // DELETE: api/sepet/sil/{id}
        [HttpDelete("sil/{id}")]
        public async Task<IActionResult> SepettenSil(int id)
        {
            try
            {
                var sepetKaydi = await _context.Sepet.FindAsync(id);
                if (sepetKaydi == null)
                {
                    return NotFound("Sepet kaydı bulunamadı");
                }

                _context.Sepet.Remove(sepetKaydi);
                await _context.SaveChangesAsync();

                _logger.LogInformation($"{id} ID'li sepet kaydı silindi");
                return Ok(new { message = "Sepet kaydı silindi" });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> SepetiTemizle(int musteriId)
        {
            try
            {
                var sepetKayitlari = await _context.Sepet
                    .Where(s => s.MusteriId == musteriId)
                    .ToListAsync();
                ...
                _context.Sepet.RemoveRange(sepetKayitlari);
                await _context.SaveChangesAsync();

                _logger.LogInformation($"{musteriId} ID'li müşterinin sepeti temizlendi. {sepetKayitlari.Count} kayıt silindi.");
        ...
        public async Task<IActionResult> GetSepetSayisi(int musteriId)
        {
            try
            {
                var sepetSayisi = await _context.Sepet
                    .Where(s => s.MusteriId == musteriId)
                    .CountAsync();
